<template>
	<view class="bmd">
		...
		<view class="form">
			<view class="username">
				<input class="inputcolor" placeholder="请输入您的手机号码" v-model="phoneNumber" placeholder-style="color:rgb(135, 135, 135);background: rgba(245, 245, 245, 1);" />
			</view>
			<view class="password">
				<input placeholder="请输入您的用户密码" v-model="passwd" password="true" placeholder-style="color:rgb(135, 135, 135);background: rgba(245, 245, 245, 1);" />
			</view>
		...
		<view class="list1">
			<view class="border">三方登录</view>
			<view class="border" @tap="toPage('binding-mobile')">用户注册</view>
			<view class="border" @tap="toPage('resetpasswd')">忘记密码</view>
		</view>
	</view>
</template>
	...
	methods: {
		...
		async doLogin() {
			...
			try {
				...
				const res = await this.$http.post('api/TokenAuth/Authenticate', data);
				this.$$.setAuthCode(res.accessToken);
				console.log(JSON.stringify(res));
				uni.setStorageSync('UserInfo', res.user);
				const userinfo = this.$$.clone(res.user);
				this.update({
					user: userinfo,
					userName: this.phoneNumber,
					token: res,
					userId: res.userId,
					phoneNubmer: this.phoneNumber,
		...
		async registerDevice() {
			try {
				const inf = plus.push.getClientInfo();
				console.log(`clientInf: ${JSON.stringify(inf)}`);
				const data = {
					devicePlatform: plus.os.name,
					deviceName: plus.device.model,
					deviceIdentifier: inf.clientid,
					deviceToken: inf.token,
					preferences: ''
				};
				console.log(`device:${JSON.stringify(data)}`);
				const res = await this.$http.post(`api/services/app/PushNotification/RegisterDevice`, data);
			} catch (error) {
				console.log(error);
			}
		}
