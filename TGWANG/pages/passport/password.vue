<template>
	<view>
		<view class="logo">
			<view class="title">2/2设置密码</view>
		</view>
		<view class="form">
			<view class="username">
				<input placeholder="请输入您的登录密码" v-model="password" password=true placeholder-style="color:rgb(135, 135, 135);" />
			</view>
			<view class="code">
				<input placeholder="请确认您的登录密码" v-model="confirmPassword" password=true placeholder-style="color:rgb(135, 135, 135);" />
			</view>
		</view>
		<view class="modify">
			<view class="btn" @tap="regist">立即注册</view>
		</view>
	</view>
</template>

<script>
	import {
		mapState,
		mapMutations
	} from 'vuex'
	// import md5 from "@/common/SDK/md5.min.js";
	export default {
		data() {
			return {
				phoneNumber: "",
				code: '',
				password: "",
				getCodeBtnColor: "#ffffff",
				getCodeisWaiting: false
			}
		},
		onLoad(option) {
			this.phoneNumber = option.phoneNumber;
			this.code = option.code;
		},
		methods: {
			...mapState(['phoneNumber', 'code']),
			async regist() {
				if (this.password.length < 6) {
					uni.showToast({
						icon: 'none',
						title: '密码最短为 6 个字符'
					});
					return;
				}
				if (this.confirmPassword != this.password) {
					uni.showToast({
						icon: 'none',
						title: '两次输入密码不一致'
					});
					return;
				}
				const data = {
					UserName: this.phoneNumber,
					password: this.password,
					sendCode: this.code,
					surname: this.phoneNumber,
				};
				uni.showLoading({
					title: '注册中...'
				}); //数据请求前loading
				const res = await this.$http.post('api/Account/Register', data);
				console.log(res);
				console.log(this.phoneNumber);
				if (res.canLogin == true) {
					this.toMain();
				} else {
					uni.showToast({
						icon: 'none',
						title: res.error.message,
					});
				}
				//1q2w3E*

			},
			toMain() {
				uni.reLaunch({
					url: '/pages/passport/login',
				});
			},
		}
	}
</script>

<style lang="scss">
	@import "../../styles/style1.scss";

	@font-face {
		font-family: "HMfont-login";
		src: url('data:application/x-font-woff2;charset=utf-8;base64,') format('woff2');
	}

	.icon {
		font-family: "HMfont-login" !important;

		font-style: normal;
		color: #ffffff;

		&.weixin {
			&:before {
				content: "\e615";
			}
		}

		&.qq {
			&:before {
				content: "\e752";
			}
		}

		&.weibo {
			&:before {
				content: "\e63d";
			}
		}

		&.xiaomi {
			&:before {
				content: "\e661";
			}
		}
	}

	.form {
		.username {
			position: relative;

			.get-code {
				position: absolute;
				display: flex;
				align-items: center;
				justify-content: center;
				right: 0;
				padding: -2upx 40upx;
				// border-left: solid 1upx #fff;
				z-index: 3;
				width: 60px;
				height: 27.5px;
				line-height: 10px;
				border-radius: 0px 5px 5px 0px;
				background-color: rgba(255, 152, 0, 1);
				color: rgba(255, 255, 255, 1);
				font-size: 10px;
				text-align: center;
				font-family: Arial;
			}
		}

		.res {
			display: flex;
			justify-content: center;
			align-items: center;
			height: 100upx;
			color: rgba($color: #ffffff, $alpha: 0.8);
		}
	}
</style>
