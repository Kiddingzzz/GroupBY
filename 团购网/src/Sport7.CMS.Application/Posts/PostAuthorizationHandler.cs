...
namespace Sport7.CMS.Posts
{
    public class PostAuthorizationHandler : AuthorizationHandler<OperationAuthorizationRequirement, Post>
    {
        ...
        {
            if (await _permissionChecker.IsGrantedAsync(context.User, CMSPermissions.Posts.Delete))
            {
                return true;
            }
        ...
        private async Task<bool> HasUpdatePermission(AuthorizationHandlerContext context, Post resource)
        {
            ...
            if (await _permissionChecker.IsGrantedAsync(context.User, CMSPermissions.Posts.Update))
            {
                return true;
            }
