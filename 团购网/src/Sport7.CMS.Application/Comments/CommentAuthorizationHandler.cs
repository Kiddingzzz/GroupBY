...
namespace Sport7.CMS.Comments
{
    public class CommentAuthorizationHandler : AuthorizationHandler<OperationAuthorizationRequirement, Comment>
    {
        ...
        {
            if (await _permissionChecker.IsGrantedAsync(context.User, CMSPermissions.Comments.Delete))
            {
                return true;
            }
        ...
        private async Task<bool> HasUpdatePermission(AuthorizationHandlerContext context, Comment resource)
        {
            ...
            if (await _permissionChecker.IsGrantedAsync(context.User, CMSPermissions.Comments.Update))
            {
                return true;
            }
